...
- **Post-QA regression fixes (manual QA validation pass 1 & 2)**:
  - Fixed verse tracker showing verse 1 after manual quick-nav selection: `_collectVerseMarkerOffsets()` now returns early (no header resync) while marker data is still partial, and `_syncTopVerseFromScroll()` now prefers the first verse below viewport top (or current cached top verse) before falling back to verse 1. This prevents transient snaps to verse 1 during highlight-triggered HtmlWidget rebuilds.
  - Fixed alphabetical quick-nav tab not auto-scrolling to current book: retained `_tabController.addListener(_onBookTabChanged)` and upgraded alphabetical scrolling to key-based `Scrollable.ensureVisible(...)` for the current book row (with controller-based fallback), eliminating row-height estimate overshoot.
